...
utf="openbill/utils/.update_shell_time"
utf2="openbill/utils/.update_shell_time2"
dirs="dpl/ openbill/"
restart=$1
if [ -z "$restart" ]; then
restart=2
...
#  restart="$1"
  ...
  list=`echo "$files" | xargs`
   touch $utf2
  (tar cvf - $list | ssh danil@orionet.ru /home/danil/projects/openbill/utils/receiveupdate.sh $restart) && mv $utf2 $utf
else
   echo "No new files.."
fi
